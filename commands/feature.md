---
description: Build something new — data model, behaviour, UI, constraints
argument-hint: <intent...>
---

The user invoked `/aipe:feature` with intent: `$ARGUMENTS`.
...
3. Print: `✓ Scaffolded .aipe/. Edit .aipe/project/context.md, then re-run /aipe:feature $ARGUMENTS.`
...
## Step 3 — Load the `feature` template

Read the template at:

```
${CLAUDE_PLUGIN_ROOT}/specs/feature.md
```

If `${CLAUDE_PLUGIN_ROOT}` is unset (running from a dev clone), fall back to searching for `specs/feature.md` upward from this file's location.
...
Path: `.aipe/specs/features/<slug>.md`

If that file already exists, append an ISO timestamp: `.aipe/specs/features/<slug>-<YYYY-MM-DDTHH-MM-SS>.md`. **Never overwrite.**
