...
The user invoked `/aipe:integration` with intent: `$ARGUMENTS`.
...
3. Print: `✓ Scaffolded .aipe/. Edit .aipe/project/context.md, then re-run /aipe:integration $ARGUMENTS.`
...
## Step 3 — Load the `integration` template

Read the template at:

```
${CLAUDE_PLUGIN_ROOT}/specs/integration.md
```

If `${CLAUDE_PLUGIN_ROOT}` is unset (running from a dev clone), fall back to searching for `specs/integration.md` upward from this file's location.
...
Path: `.aipe/specs/integrations/<slug>.md`

If that file already exists, append an ISO timestamp: `.aipe/specs/integrations/<slug>-<YYYY-MM-DDTHH-MM-SS>.md`. **Never overwrite.**
