---
description: Rewrite tasks in different personas
argument-hint: <intent...>
---

The user invoked `/aipe:user-stories` with intent: `$ARGUMENTS`.
...
3. Print: `✓ Scaffolded .aipe/. Edit .aipe/project/context.md, then re-run /aipe:user-stories $ARGUMENTS.`
...
## Step 3 — Load the `user-stories` template

Read the template at:

```
${CLAUDE_PLUGIN_ROOT}/specs/user-stories.md
```

If `${CLAUDE_PLUGIN_ROOT}` is unset (running from a dev clone), fall back to searching for `specs/user-stories.md` upward from this file's location.
...
Path: `.aipe/specs/user-stories/<slug>.md`

If that file already exists, append an ISO timestamp: `.aipe/specs/user-stories/<slug>-<YYYY-MM-DDTHH-MM-SS>.md`. **Never overwrite.**
