---
description: Bug keeps coming back — instrument, observe, fix from evidence
argument-hint: <intent...>
---

The user invoked `/aipe:debugging` with intent: `$ARGUMENTS`.
...
3. Print: `✓ Scaffolded .aipe/. Edit .aipe/project/context.md, then re-run /aipe:debugging $ARGUMENTS.`
...
## Step 3 — Load the `debugging` template

Read the template at:

```
${CLAUDE_PLUGIN_ROOT}/specs/debugging.md
```

If `${CLAUDE_PLUGIN_ROOT}` is unset (running from a dev clone), fall back to searching for `specs/debugging.md` upward from this file's location.
...
Path: `.aipe/specs/bugs/<slug>.md`

If that file already exists, append an ISO timestamp: `.aipe/specs/bugs/<slug>-<YYYY-MM-DDTHH-MM-SS>.md`. **Never overwrite.**
