...
- **`format.md`** — the concept-file template, house-style traits, diagram rules, pseudocode rules, hard rules. **Per format.md, the concept-file blocks are:** Subtitle → Zoom out, then zoom in → How it works (carries pattern teaching AND code side-by-side + annotation) → Primary diagram → Elaborate → Project exercises (AI/ML only) → Interview defense → See also. **Removed from older templates: Why care (replaced by Zoom out), Tradeoffs, Tech reference, Summary, Implementation in codebase (folded into How it works Move 2).** Where legacy template sections in `study-ai-engineering.md`'s body conflict with format.md, format.md wins.
...
- **Template drift** — `format.md` has added new blocks (e.g., a new Move sub-section, a new Tech reference field) since the file was written. Identify missing blocks.
