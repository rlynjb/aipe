...
This command produces a topic-focused companion to `/aipe:study` — same per-concept template, same staff-engineer persona, but a fixed concept inventory spanning AI engineering (LLM foundations, retrieval, agents, evals, production serving) and machine learning. **AI engineering and ML content lives exclusively here** — the per-codebase study generators (`study-system-design-dsa`, `study-ai-engineering`, etc.) are each scoped to a single topic, with format.md providing the shared structure across all of them.
