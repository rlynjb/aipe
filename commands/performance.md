---
description: Diagnose speed or bundle size issues
argument-hint: <intent...>
---

The user invoked `/aipe:performance` with intent: `$ARGUMENTS`.
...
3. Print: `✓ Scaffolded .aipe/. Edit .aipe/project/context.md, then re-run /aipe:performance $ARGUMENTS.`
...
## Step 3 — Load the `performance` template

Read the template at:

```
${CLAUDE_PLUGIN_ROOT}/specs/performance.md
```

If `${CLAUDE_PLUGIN_ROOT}` is unset (running from a dev clone), fall back to searching for `specs/performance.md` upward from this file's location.
...
Path: `.aipe/specs/performance/<slug>.md`

If that file already exists, append an ISO timestamp: `.aipe/specs/performance/<slug>-<YYYY-MM-DDTHH-MM-SS>.md`. **Never overwrite.**
