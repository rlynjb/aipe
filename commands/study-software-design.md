...
1. **All structural rules from `format.md` apply** — the per-concept blocks (Subtitle, Zoom out / then zoom in with the LAYERS diagram, Structure pass, How it works, Primary diagram, Implementation in codebase, Elaborate, Interview defense, See also). Formatting rules and box-drawing diagram chars.
