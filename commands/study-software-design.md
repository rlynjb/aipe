...
1. **All structural rules from `format.md` apply** — the per-concept blocks (Subtitle, Zoom out / then zoom in with the LAYERS diagram, Structure pass, How it works (with code side-by-side + annotation inside Move 2), Primary diagram, Elaborate, Interview defense, See also). Formatting rules and box-drawing diagram chars.
2. **How it works (Move 2) is the heavy block** — this is the audit. Real files, real line ranges, deep vs shallow examples, the red flag firing or not, and the fix, shown side-by-side with annotation inside the pattern walkthrough. Every finding cites `**File:**` + `**Function / class:**` + `**Line range:**`. **Never drop a raw block without annotation.**
...
Regenerate `audit.md` against current evidence. Add new Pass 2 pattern files for newly-load-bearing design moves. Update existing pattern files where the implementation changed. Remove pattern files only when the design move is genuinely gone from the codebase (not just refactored).
