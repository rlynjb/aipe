...
12b. **Use frontend primitives the reader builds with — not analogies, not whole products.** Whenever the writer needs a concrete anchor, reach for the lowest-level primitive that still carries the point. The reader is a working frontend engineer with 5–8 years of experience; their pattern recognition is richest from the *substrate* they build with daily (a todo list, a DB table, a `.map()` with `key`), less rich from specific products they may or may not use (Linear, Notion, Gmail), and weakest from physical-world metaphors.

    **Banned as primary anchors:** coat checks, librarians, locked doors, bouncers, matchmakers, post offices, notebooks, kitchens, factories, queues at coffee shops, traffic lights, mailboxes — and also **whole-product references** ("Linear does X", "GitHub does Y") *when a lower-level primitive captures the same point*.

    **Preferred anchor types, in priority order — reach for the most universal one that works:**

    1. **Frontend primitives the reader builds with every day** (the default). The most universal anchor, because every frontend engineer has used these regardless of which products or stack they work in. A todo list rendering on screen. A database table with rows and columns. A `.map()` with a `key` prop. A form input and its controlled value. A `fetch()` and its loading / error / success states. A list re-rendering after `setState`. A primary key. A `WHERE` clause. A query string. A session cookie.
    2. **Patterns the reader has built in their own apps.** "You already do this when you check the logged-in user in the route handler AND filter by `user_id` in the query — that's two gates." "You already do this when you set `key` on a list item."
    3. **DevTools and engineering surfaces the reader touches.** The network tab. The "missing key prop" console warning. A response header like `X-RateLimit-Remaining`. The session cookie in application storage.
    4. **Industry-standard protocols and primitives.** JWT signatures. OAuth's authorization-code flow. Postgres `MVCC`. Redis `SETNX`. HTTP keep-alive. ETag headers. Use these only when no lower-level primitive captures the concept.
    5. **Whole products — last resort.** "Gmail's optimistic send", "Linear's drag-to-reorder", "GitHub's branch protection." Avoid these when a primitive works: they assume the reader uses that specific product. A todo list and a DB table are universal; a SaaS product is not. Use a whole-product anchor only when the concept genuinely has no primitive-level equivalent.

    **The test (two parts):**
    1. Could the reader open the app, browser tool, or codebase and verify what the writer claims? If only "trust me, this is like a coat check" works, replace with a real example.
    2. Is there a more universal anchor one level down? If the writer reached for "Linear's drag-to-reorder" but a todo list with `.map(todo => <Row key={todo.id}>)` would carry the same point, drop to the todo list.

    **Narrow exception:** when the only working example involves software the reader is unlikely to have touched (specialized academic systems, internal-only tooling at large companies), a brief physical-world reference is acceptable as a *secondary* anchor *after* the engineering reference. Never as the primary. Note the exception in the file itself so the reviewer knows it was deliberate.
...
13. **Every concept file includes a "Why care" block** immediately after `**See also:**` and BEFORE `## How it works`. **Length scales with complexity** — a simple concept gets a short scenario plus a one-line summary; a complex one gets a fuller scenario, a worked before/after, and a bolded "why this matters" pivot. The block has **five structural moves**, in order:
    ...
    - **Move 1 — The grounded scenario.** Open with a concrete picture the reader can hold in their head — built from **the primitives a frontend engineer touches every single day**: a todo list rendering, a `.map()` with `key`, a database table with rows and columns, a form input, a `fetch()` and its loading/error/success states, a list re-rendering after `setState`. **Not abstract framing. Not a clever hook sentence. Not a physical-world analogy. Not a whole-product reference** ("Linear's drag-to-reorder", "Gmail's optimistic send") when a lower-level primitive works. Examples:
      - "You've got a todo list on screen — five rows, each with text the user typed. The user fixes a typo in row 3 and the list re-renders. To the user it's the same five todos. But to the code, is row 3 the *same* row 3, or a brand-new row that happens to sit in the same spot?"
      - "Picture a database table: `todos`, with columns `id`, `text`, `created_at`, `user_id`. Two different users each have a row with `id = 1`. When the app asks for 'todo number 1,' which row comes back — and what stops it from being the wrong one?"
      - "You render a list of todos with `.map()`. Every React dev sets `key={todo.id}` because the console warns if you don't. But strip the `key` out and watch what breaks: the input you were typing in loses focus, the half-finished animation restarts. Something was relying on that key."

      The reader is looking at *the substrate of their own work*, not imagining a kitchen, a library, OR a specific SaaS product they may not use. Todo lists and DB tables are universal; Linear isn't. **Hook-sentence openings are banned**; **physical-world analogies are banned**; **whole-product references are banned as primary anchors when a primitive captures the point**. **Move 1 must be project-agnostic** — a reader who has never seen this codebase must understand the scenario.
    ...
    - **Move 5 — The one-line summary.** End with **one sentence** that names the pattern in a single phrase — a *reduction*, not a definition, and **not a physical-world metaphor**. Compress the concept by reference to a frontend primitive the reader builds with. Examples:
      - "The reconciler is what React's `key` prop does, but for database rows with attached columns instead of DOM nodes with attached state."
      - "The composite key is just a primary key made of two columns instead of one — `(user_id, id)` instead of `id`."
      - "Optimistic update is rendering the new todo in the list before the `POST` comes back, and rolling the row back out if it fails."

      Prefer primitive-level summaries (a `key` prop, a primary key, an optimistic list render) over whole-product references ("Linear's checkmark"). This is the sentence the reader paraphrases three weeks later, anchored to the substrate of their own work. Optionally follow with one handoff line to How it works ("Here's how that actually plays out in this codebase." / "The full mechanics are below.").
    ...
    **Banned phrases throughout the block:** "It's important to understand X" / "X is a fundamental concept" / "X is fascinating because" — all permission-seeking. The reader gives permission by reading the next sentence; trust the scenario to earn the attention.
    ...
    The reader profile is the same as How it works: a working frontend engineer (5–8 years React/Vue/TS) pivoting toward full-stack and AI. Move 1's scenario must connect to software they already use or patterns they've already built: Linear, Notion, Gmail, GitHub, ChatGPT, the network tab in DevTools, the `key` warning in React, a route handler they've written that checks `req.user.id`. Not "imagine a library." Not "imagine you're scaling a Kubernetes cluster." A full worked example showing what good looks like lives in the loaded template (Step 3) under "Why care → Worked example — what good looks like."
...
18. **How it works is the load-bearing block; it follows a three-move structure with frontend bridging.** Length scales with complexity, not capped at a paragraph count. A simple concept (debounce) gets four short paragraphs; a complex backend/AI/infra concept (multi-layer auth, distributed locks, prompt routing under failure) gets fifteen to twenty paragraphs with sub-headings, interspersed visuals, and explicit current-state vs future-state walkthroughs. The reader profile is a working frontend engineer (5–8 years React/Vue/TypeScript) pivoting toward full-stack and AI engineering — comfortable with components, hooks, client state, forms, browser APIs; still building intuition for database internals, transactional semantics, server-side request handling, auth flows, queues, distributed systems, LLM-shaped failure modes. The writer's job is to bridge from the first list to the second.
    ...
    - **Move 1 — The mental model (first paragraph + diagram).** Open with **a concrete picture anchored to a frontend primitive the reader builds with** (a re-rendering list, a DB table, a primary key, a `fetch()` and its states) — NOT a definition, NOT a physical-world analogy, NOT a whole-product reference when a primitive works (see rule 12b). Examples:
      - Defense in depth → "Two WHERE conditions on the same query, where either one alone would still scope the rows correctly. If the first is bypassed, the second catches it."
      - Connection pooling → "The same open database connection handed to one request after another, instead of opening a fresh one each time — the way a single `fetch` keep-alive connection serves many calls."
      - Optimistic UI → "Rendering the new row in the list the instant the user hits save, before the `POST` resolves, and removing it again only if the request fails."

      After the mental model lands, one sentence names the underlying strategy in plain English ("two independent gates, both required" / "one warm resource, lent and returned" / "show success immediately, reconcile when the server confirms").

      **Diagram required.** Move 1 always includes one small ASCII diagram (5–12 lines) showing the literal shape of the mental model — the two WHERE-clauses as boxes the query passes through, the one connection with many request arrows, the new row appearing before the response arrow. The diagram is the visual the reader will recall alongside the sentence.

    - **Diagrams required at every Move 2 sub-section (and Move 2.5).** Move 2 is a layered walkthrough where each independent moving part gets its own bolded sub-heading. **Each sub-section requires at least one ASCII diagram** showing its mechanism — not "where it helps", every sub-section. Move 2.5 (Phase A/B) requires a side-by-side comparison diagram. Pick from five diagram types:
      - **Flow / pipeline** — sequence of steps, request through stages. Boxes connected by labelled arrows, top to bottom.
      - **Table rows & columns** — what's stored, the shape of a record, how rows relate. Use this constantly; the reader thinks in tables.
      - **Comparison / side-by-side** — before vs after, with-pattern vs without, Phase A vs Phase B. Two columns, aligned rows, the difference marked.
      - **Sequence / interaction** — actors exchanging messages over time (client ↔ server ↔ database). Actors as columns, time flowing down, labelled arrows.
      - **Inline code annotation** — pointing at parts of a small snippet, naming what each piece does.

      Every diagram is **wrapped in prose**: one sentence introduces it, one sentence follows it. A diagram never stands alone. Use box-drawing characters (`┌─│▼◄►`), not ASCII approximations (`+−|v<`). Label every box, every information-carrying arrow, every layer the system spans (UI, Service, Storage). No Mermaid, no images.
...
Reader profile (same as How it works): a working frontend engineer (5–8 years React/Vue/TypeScript) pivoting toward full-stack and AI. Move 1's scenario must connect to software they already use or patterns they've already built — Linear, Notion, Gmail, GitHub, ChatGPT, the network tab in DevTools, the `key` warning in React, a route handler they've written that checks `req.user.id`. **Not "imagine a library." Not a notebook page, not a coat check, not a coffee-shop queue** — see non-negotiable 12b for the ban on physical-world analogies and the catalog of preferred anchors.

The block has **five structural moves**, in order:

### Move 1 — The grounded scenario

Open with a concrete picture the reader can hold in their head — built from **a primitive a frontend engineer touches every single day**. Not abstract framing. Not a clever hook sentence. **Not a physical-world analogy. Not a whole-product reference** ("Linear's drag-to-reorder", "Gmail's optimistic send") when a lower-level primitive carries the same point.

Examples of opening scenarios that work:

- "You've got a todo list on screen — five rows, each with text the user typed. The user fixes a typo in row 3 and the list re-renders. To the user it's the same five todos. But to the code, is row 3 the *same* row 3, or a brand-new row that happens to sit in the same spot?"
- "Picture a database table: `todos`, with columns `id`, `text`, `created_at`, `user_id`. Two different users each have a row with `id = 1`. When the app asks for 'todo number 1,' which row comes back — and what stops it from being the wrong one?"
- "You render a list of todos with `.map()`. Every React dev sets `key={todo.id}` because the console warns if you don't. But strip the `key` out and watch what breaks: the input you were typing in loses focus, the half-finished animation restarts. Something was relying on that key to know which row was which."

What these scenarios have in common: they reference **things the reader builds with daily** — a rendered list, a table with named columns, a `.map()` with a `key`. The reader isn't imagining a kitchen or recalling a product they might not use; they're looking at the substrate of their own work.

**Hook-sentence openings are banned.** "Most of the speed in a modern web app comes from not doing work" is clever but doesn't put a picture in the reader's head. **Physical-world analogies are banned** (coat checks, libraries, locked doors, post offices — see rule 12b). **Whole-product anchors are banned as primary** when a primitive captures the point — "Linear does X" assumes Linear; a todo list assumes only that the reader has built a list. **Move 1 must be project-agnostic** — a reader who has never seen this codebase must understand the scenario.
...
> "That same-row-or-new-row question is what a reconciler answers. Not the diffing of the array, not the re-rendering — just the matching of old-row to new-row. It's the exact job React's `key` prop does on the frontend. Backends hit the same question and reach for the same pattern."

> "What stops 'todo number 1' from being the wrong row is the *composite key* — the table is keyed on `(user_id, id)` together, not `id` alone. The pair is what's unique, so user A's row 1 and user B's row 1 are simply different rows."

> "What keeps the half-finished input from losing focus is *identity tracking* — the list keeps a stable handle on each row across re-renders, so the DOM node, its focus, and its state all stay attached to the same logical item."

Sharpening technique: **name what the pattern is by naming what it is NOT**. "Not the diffing of the array, not the re-rendering — just the matching." Use it when the pattern sits adjacent to similar patterns it gets confused with.
...
End with **one sentence** that names the pattern in a single phrase — a **reduction**, not a definition, and **not a physical-world metaphor** (no matchmakers, no bouncers, no coat checks). Compress the concept by reference to **a frontend primitive the reader builds with**. Prefer primitive-level summaries over whole-product references.

> "The reconciler is what React's `key` prop does, but for database rows with attached columns instead of DOM nodes with attached state."
> "The composite key is just a primary key made of two columns instead of one — `(user_id, id)` instead of `id`."
> "Optimistic update is rendering the new todo in the list before the `POST` comes back, and rolling the row back out if it fails."

This is the sentence the reader will paraphrase three weeks later, anchored to the substrate of their own work. Optionally follow with one **handoff line** that points to How it works: "Here's how that actually plays out in this codebase." / "The full mechanics are below."

### Hard rules (enforce throughout the block)

- **Open with a scenario, not a hook sentence.** "Open DevTools' network tab on any production app and you'll see the same TCP connection serving fifty requests in a row — that's not the default, that's keep-alive earning its place" puts a picture in the reader's head; "Most of the speed in a modern web app comes from not doing work" doesn't.
- **No physical-world analogies.** Coat checks, librarians, post offices, locked doors, bouncers, kitchens — all banned as primary anchors. See non-negotiable 12b for what to reach for instead (real apps, DevTools surfaces, patterns the reader has built, industry-standard primitives).
...
- **End with the one-line summary.** Move 5 is one sentence the reader can lift verbatim to describe the pattern, by reference to real software they already know.

### Worked example — what good looks like

The concept being explained: *reconciliation between an old list and a new list when rows have metadata attached* — a pattern the reader has already used on the frontend (via React's `key` prop) but probably never named.
...
> ```
>
> You set `key={todo.id}` because if you don't, React will warn you in the console. But why does React need the key at all? Open the dev tools, drag a row to reorder the list, and watch what happens: the input cursor inside the dragged row stays in the same field, the focused element stays focused, the animation continues smoothly. Without `key`, all of that breaks — inputs lose focus, animations restart, state resets. The key is how React matches each item in the new list to the same item in the old list, so attached state can ride along.
>
> That matching problem is what a *reconciler* solves. Not the diffing of the array. Not the re-rendering. Just the matching of new-row to old-row.
>
> **Why you need to answer that question at all:** because *stuff is attached to the old rows*. In React, the attached thing is component state and DOM focus. In a backend with persistent rows, the attached thing is server-side metadata — created-at timestamps, AI classifications, tags, thread links. That metadata lives with the row's identity, not its text. If the user fixes a typo in row 3, you want row 3 to still be row 3 — same created-at, same tags, same classification. You don't want the system to think "row 3 was deleted and a new row was inserted," because then the created-at resets to now, the tags vanish, and the AI re-classifies from scratch.
>
> So the backend reconciler exists for the same reason React's reconciler does — to prevent that loss. It looks at old-list and new-list and says "these two rows are the same row, just edited" so the metadata can ride along.
>
> Without a reconciler:
> - User fixes typo in todo #3
> - Backend sees "the list changed" → wipes old rows, inserts new rows
> - Todo #3's created-at is now today, its tags are gone, its AI classification re-runs and maybe comes back different
>
> With a reconciler:
> - User fixes typo in todo #3
...
> - Metadata stays attached, only the text field updates
>
> The backend reconciler is React's `key` prop, but for server-side rows with attached metadata. Everything else — the storage, the UI, the classifier — depends on it getting the matches right. Here's how that actually plays out in this codebase.

What this example does right: opens with React code the reader has written thousands of times — no notebook, no librarian, a real verifiable pattern; names the pattern (reconciler) by reference to React's reconciler (which the reader has used even if they didn't know the term); bolded transition to "why answering that question matters"; concrete consequences with specific field names; before/after walks the difference; one-line summary closes with a real-software reference ("the backend reconciler is React's `key` prop, but for server-side rows") and a handoff sentence.
...
Open with **a concrete picture anchored to a frontend primitive the reader builds with** (a re-rendering list, a DB table with rows and columns, a primary key, a `fetch()` and its loading/error/success states) — NOT a definition, NOT a physical-world analogy, NOT a whole-product reference when a primitive works. Examples:
...
**Banned: physical-world analogies** (locked doors, coat checks, librarians, post offices). **Also banned: whole-product anchors** (`Linear's optimistic checkmark`, `GitHub's branch protection`) when a primitive captures the same point. See non-negotiable 12b for the priority order.

**Diagram required: the mental-model mnemonic.** Move 1 always includes one small ASCII diagram (5–12 lines) — the literal shape of the mental model. If the model is "two WHERE conditions, either one sufficient," draw the two conditions as two boxes the query passes through. If it's "one connection handed to many requests," draw the one connection with many request arrows pointing at it. Place it immediately after the opening paragraph.
...
**Every Move 2 sub-section requires at least one ASCII diagram showing its mechanism.** Not "where it helps" — every sub-section. The prose explains; the diagram shows. A reader who skims only the diagrams in Move 2 should come away with the shape of each mechanism before reading a sentence. Most sub-sections need more than one diagram — typically a data-shape diagram (what's stored) plus a flow diagram (how the operation moves).

Pick the diagram type that matches what the sub-section is describing:

- **Flow / pipeline** — a sequence of steps, a request moving through stages, a transformation. Boxes connected by labelled arrows, top to bottom.
- **Table rows & columns** — what's stored, the shape of a record, how rows relate. Use this constantly; the reader thinks in tables.
- **Comparison / side-by-side** — before vs after, with-pattern vs without, Phase A vs Phase B. Two columns, aligned rows, the difference marked.
- **Sequence / interaction** — actors exchanging messages over time (client ↔ server ↔ database). Actors as columns, time flowing down, labelled arrows between.
- **Inline code annotation** — pointing at parts of a small snippet, naming what each piece does.

Rules for every diagram: label every box, label every information-carrying arrow, name every layer the system spans (UI, Service, Storage). Use box-drawing characters (`┌─│▼◄►`), not ASCII approximations (`+−|v<`). Introduce each diagram with one sentence of prose before it and one after — **a diagram never stands alone**. No Mermaid, no images.
...
**Diagram required: side-by-side comparison.** Move 2.5 always includes one comparison diagram — Phase A on the left, Phase B on the right, with the elements that change marked (an arrow, a `(new!)` label, a highlighted box). The reader sees in one glance what stays the same and what changes:
...
- **Diagrams at every move.** Move 1 gets a mnemonic diagram of the mental model. Every Move 2 sub-section gets at least one mechanism diagram. Move 2.5 gets a side-by-side comparison. A How it works block with prose-only sub-sections is incomplete — the diagrams aren't decoration, they carry the mechanics. Every diagram is wrapped in prose: one sentence before, one after.
...
- **Sections structurally absent** — required template elements not present in the file at all. The current required structure, in order:
  ...
  4. `**See also:**` line
  5. `## Why care` (restructured in v1.30.0; sharpened in v1.31.0; sharpened again in v1.32.0 — anchors drop from "real software" (Linear, Gmail, GitHub) to **frontend primitives the reader builds with** (a todo list, a DB table with rows and columns, `.map()` with `key`). Whole-product references are now banned as primary anchors when a lower-level primitive captures the same point. Five structural moves: Move 1 grounded scenario anchored to a frontend primitive, project-agnostic; Move 2 name the question/pattern, framed as the answer, often by reference to a primitive ("it's what `key` prop does, but for backend rows"); Move 3 bolded transition `**Why you need to answer that question at all:**` then concrete-stakes paragraph; Move 4 optional concrete before/after; Move 5 one-line summary that names the pattern by reference to a primitive ("The composite key is just a primary key made of two columns instead of one"). Hook-sentence openings, definition-first openings, physical-world analogies, AND whole-product references (when a primitive works) are all banned. Banned phrases: "It's important to understand X", "X is a fundamental concept", "X is fascinating because.")
  6. `## How it works` (moved BEFORE the primary diagram in v1.20.0; restructured in v1.24.0; sharpened in v1.31.0; sharpened again in v1.32.0 — anchors drop to frontend primitives, AND diagrams are now required at every move. Length scales with complexity. Required structure: Move 1 — mental model anchored to a frontend primitive (a re-rendering list, a DB table, a primary key, a `fetch()` and its states), plus a mnemonic ASCII diagram (5–12 lines) showing the literal shape; Move 2 — layered walkthrough where each independent moving part gets its own bolded sub-heading covering four things (technical term, frontend bridge, concrete consequence, boundary condition), AND **at least one ASCII diagram per sub-section** showing the mechanism (flow / table rows & columns / comparison / sequence / inline code annotation); Move 2.5 — Phase A / Phase B sub-section when applicable, plus a side-by-side comparison diagram; Move 3 — the principle that generalises (NOT a summary). Every move-2 sub-heading must include a frontend-to-backend bridge sentence. Every abstract claim followed by a concrete consequence. **Every diagram wrapped in prose** — one sentence before, one after. Box-drawing characters required; label every box and every information-carrying arrow; name every layer. Whole-product anchors ("Linear does X") are banned as primary when a primitive works. Ends with handoff to the primary diagram.)
  ...
  - "Why care opens with a hook sentence, not a scenario" — flag any Move 1 that opens with a clever framing instead of a concrete scenario the reader can visualize ("Most of the speed in a modern web app comes from not doing work" / "X is fascinating because…" / definition-first openings). The fix is to rewrite Move 1 anchored to real software the reader uses (Linear's drag-to-reorder, Gmail's optimistic send, GitHub's branch protection, the reader's own React code) — see flag below for the additional ban on physical-world analogies.
  - "Why care or How it works uses a physical-world analogy as the primary anchor" — flag any Move 1 (Why care) or Move 1 (How it works) anchored to coat checks, librarians, locked doors, post offices, bouncers, kitchens, factories, notebooks, traffic lights, or any other physical-world metaphor. The v1.31.0 rule (non-negotiable 12b) bans these as primary anchors. The fix is to rewrite the anchor using a frontend primitive the reader builds with (a todo list, a DB table, a `.map()` with `key`, a `fetch()`).
  - "Why care or How it works uses a whole-product anchor when a primitive works" (v1.32.0) — flag any Move 1 anchored to "Linear's drag-to-reorder", "Gmail's optimistic send", "GitHub's branch protection", "Notion's per-user workspace", "ChatGPT's persistent system prompt" or similar SaaS-product reference when a lower-level frontend primitive would carry the same point. The v1.32.0 rule (non-negotiable 12b priority order) puts whole-product references at level 5 (last resort) and prefers level 1 (frontend primitives the reader builds with). The fix is to drop down a level: "Linear's drag-to-reorder" → "a todo list re-rendering after the user fixes a typo"; "Gmail's optimistic send" → "rendering the new row in the list before the `POST` resolves"; "GitHub's branch protection" → "two WHERE conditions on the same query".
  - "How it works Move 1 missing mnemonic diagram" (v1.32.0) — flag any `## How it works` block whose Move 1 (mental model paragraph) doesn't include a small ASCII diagram (5–12 lines) showing the literal shape of the model. Move 1 always has both a sentence and a diagram. The fix is to add the diagram immediately after the opening paragraph.
  - "How it works Move 2 sub-section missing mechanism diagram" (v1.32.0) — flag any move-2 sub-heading whose body is prose-only without at least one ASCII diagram showing the mechanism. The v1.32.0 rule requires every Move 2 sub-section to carry a diagram (flow / table rows & columns / comparison / sequence / inline code annotation). The fix is to add at least one diagram per sub-section, wrapped in prose (one sentence before, one after).
  - "How it works Move 2.5 (Phase A/B) missing side-by-side diagram" (v1.32.0) — flag any Phase A / Phase B sub-section that's prose-only. The fix is to add a side-by-side comparison diagram with Phase A on the left, Phase B on the right, and changed elements marked (`(new!)`, an arrow, a highlighted box).
  - "Diagram stands alone without prose wrap" (v1.32.0) — flag any ASCII diagram inside Why care or How it works that isn't wrapped in prose: one sentence before introducing it, one sentence after. Diagrams never stand alone. The fix is to add the bracketing sentences.
  - "Why care Move 5 uses a metaphor instead of a real-software summary" — flag any Move 5 phrased as a metaphor ("The reconciler is just the matchmaker." / "The lock is the bouncer at the door." / "The connection pool is the coat check."). The v1.31.0 rule renames Move 5 from "metaphor" to "summary" and requires the one-line phrase to name the pattern by reference to real software the reader already knows ("The reconciler is what React's `key` prop does, but for backend rows." / "The lock is `SETNX`, exactly that." / "The connection pool is HTTP keep-alive applied to database sockets.").
  ...
  - "Why care missing one-line summary (Move 5)" — flag any Why care block that ends without a single-sentence summary naming the pattern in one phrase by reference to real software ("The reconciler is what React's `key` prop does, but for backend rows."). The fix is to append the summary; this is the sentence the reader paraphrases three weeks later, anchored to something they already know.
  ...
  - **Note on legacy guides:**
    ...
    - Files generated under **v1.30.0** likely use **physical-world analogies** in Why care Move 1 and Move 5, and in How it works Move 1 (the mental model). v1.31.0 bans physical-world analogies as primary anchors (see non-negotiable 12b) and renames Why care Move 5 from "metaphor" to "summary". The v1.31.0 fix is to walk every Why care and How it works block in the existing guide and replace physical-world anchors with real-software references: notebooks/coat checks/librarians/locked doors/post offices/bouncers/matchmakers/kitchens → apps the reader uses (Linear, Gmail, GitHub, Notion, ChatGPT), DevTools surfaces (the network tab, the React `key` warning, `X-RateLimit-Remaining`), patterns the reader has built (`req.user.id` checks, React `key` props), or industry primitives (`SETNX`, JWT signatures, HTTP keep-alive, Postgres MVCC). Move 5 specifically: the one-line summary should name the pattern by reference to a software pattern the reader already knows — "The reconciler is what React's `key` prop does, but for backend rows" not "The reconciler is just the matchmaker." The substitution table in the corresponding Step 8U repair recipe lists common analogy-to-real-software conversions.
    - Files generated under **v1.31.0** likely use **whole-product anchors** (Linear's drag-to-reorder, Gmail's optimistic send, GitHub's branch protection) as primary anchors AND lack **diagrams at every Move 2 sub-section / at Move 2.5**. v1.32.0 sharpens both: (a) anchors drop from whole-product references (level 5 in the new priority order) to **frontend primitives the reader builds with** (level 1 — a todo list, a DB table, `.map()` with `key`); (b) **diagrams required at every move in How it works** — Move 1 mnemonic, every Move 2 sub-section, Move 2.5 side-by-side. The v1.32.0 fix is to: (1) walk every Why care and How it works anchor and check whether a frontend primitive captures the same point — if yes, drop down ("Linear's drag-to-reorder" → "a todo list re-rendering after the user fixes a typo"; "GitHub's branch protection" → "two WHERE conditions on the same query"); (2) walk every How it works block and add a mnemonic ASCII diagram (5–12 lines) after the Move 1 opening paragraph; (3) walk every Move 2 sub-section and add at least one mechanism diagram (flow / table rows & columns / comparison / sequence / inline code annotation); (4) walk every Move 2.5 sub-section and add a side-by-side comparison diagram. Wrap every diagram in prose — one sentence before, one after.
...
- For a **Why care that opens with a hook sentence instead of a scenario**, rewrite Move 1 anchored to real software the reader uses (per non-negotiable 12b). Take the hook's underlying idea, convert it into a scenario the reader can verify by opening an app. "Most of the speed in a modern web app comes from not doing work" → "Open DevTools' network tab on any production app and you'll see the same TCP connection serving fifty requests in a row — that's not the default, that's HTTP keep-alive earning its place." The scenario form anchored to real software is non-negotiable in v1.31.0.
- For a **Why care or How it works using a physical-world analogy as the primary anchor** (locked doors, coat checks, librarians, post offices, bouncers, kitchens, notebooks, factories), rewrite the anchor using a **frontend primitive the reader builds with** (the v1.32.0 priority — drop two levels, not one). Substitution table:
  - "Two locked doors" → "Two WHERE conditions on the same query, either one sufficient" (defense in depth)
  - "A coat check" → "One open connection handed to many requests, the way a single `fetch` keep-alive serves many calls" (connection pooling)
  - "A librarian with cards" → same as above
  - "The matchmaker" → "React's `key` prop, applied to database rows" (reconciler)
  - "The bouncer at the door" → "A `SELECT … FOR UPDATE` that only one transaction can hold at a time" (lock)
  - "Marking the envelope as sent" → "Rendering the new row in the list before the `POST` resolves" (optimistic UI)
  - "A notebook page being rewritten" → "A list re-rendering after `setState` — same logical row, different content" (list reconciliation)

  The test: could the reader open the app, a browser tool, or their own codebase and verify the anchor? If yes, keep it. If only the writer can confirm "trust me, this is like a coat check," replace it.
- For a **Why care or How it works using a whole-product anchor when a primitive works** (v1.32.0), rewrite the anchor one level down — from a SaaS product to a primitive every frontend engineer has built with. Substitution table:
  - "Linear's drag-to-reorder" → "A todo list where the user drags a row to a new position and the list re-renders"
  - "Gmail's optimistic send" → "Rendering the new row in the list the instant the user hits save, before the `POST` resolves"
  - "GitHub's branch protection" → "Two WHERE conditions on the same query, either one sufficient to scope the rows"
  - "Notion's per-user workspace" → "A `WHERE user_id = $1` clause auto-applied to every query"
  - "ChatGPT's persistent system prompt" → "A `systemMessage` constant prepended to every API call"

  The test: is there a more universal anchor one level down? Every frontend engineer has built a todo list and a DB table; not every engineer uses Linear. Drop to the universal anchor unless the whole-product reference genuinely has no primitive equivalent.
- For a **How it works Move 1 missing the mnemonic diagram** (v1.32.0), add one small ASCII diagram (5–12 lines) immediately after the mental-model opening paragraph. The diagram is the literal shape of the model: if the model is "two WHERE conditions, either sufficient", draw the two conditions as boxes the query passes through; if it's "one connection serving many requests", draw the one connection with many request arrows. Wrap with a sentence before and a sentence after. Use box-drawing characters; no Mermaid.
- For a **How it works Move 2 sub-section missing the mechanism diagram** (v1.32.0), add at least one ASCII diagram showing what the sub-section describes. Pick the type that fits: **flow** for sequences of steps, **table rows & columns** for what's stored, **comparison / side-by-side** for before/after or with/without, **sequence / interaction** for actors exchanging messages over time, **inline code annotation** for pointing at parts of a small snippet. Most sub-sections benefit from two diagrams — a data-shape (what's stored) plus a flow (how the operation moves). Wrap each diagram in prose (one sentence before, one after).
- For a **How it works Move 2.5 (Phase A/B) missing the side-by-side diagram** (v1.32.0), add a comparison diagram with Phase A on the left, Phase B on the right, changed elements marked (`(new!)`, arrows, highlighted boxes). The reader should see in one glance what stays the same and what changes between phases.
- For a **diagram standing alone without prose wrap** (v1.32.0), add the bracketing sentences: one sentence before introducing what the diagram shows, one sentence after pointing the reader to the next idea. Diagrams in How it works and Why care never stand alone; they're integrated into the prose flow.
- For a **Why care Move 5 phrased as a metaphor** ("The reconciler is just the matchmaker." / "The lock is the bouncer at the door."), rewrite as a real-software summary. The substitution: pick a software pattern the reader already knows from frontend, browser, or industry-standard tooling and name the new pattern by reference to it. ("The reconciler is what React's `key` prop does, but for backend rows with attached metadata." / "The lock is `SETNX` — exactly that.") The summary is the sentence the reader paraphrases three weeks later; anchoring it to real software makes the recall durable.
...
- For a **Why care missing Move 5 (the one-line summary)**, append a single sentence naming the pattern in one phrase by reference to real software the reader already knows. The pattern is what — in three weeks the reader will paraphrase about this concept, anchored to something they recognize? That sentence is Move 5. Physical-world metaphors are banned (see non-negotiable 12b).
...
- For a **`## How it works` block opening with a definition** ("X is a mechanism for...", "X is a pattern that..."), rewrite the opening paragraph to start with a mental model **anchored to real software the reader has used or built** (per non-negotiable 12b). Pick an anchor that maps to the underlying strategy: "GitHub's branch protection: two independent gates that both have to pass — review approval AND status checks" for defense in depth; "browser HTTP keep-alive, but for database sockets — same TCP connection reused across many requests instead of handshaking every time" for connection pooling; "Linear's optimistic checkmark, before the server has confirmed" for optimistic UI. After the mental model, follow with one sentence naming the underlying strategy in plain English. The technical term gets introduced inside the move-2 sub-headings, not in the opening. **Physical-world analogies (locked doors, coat checks, post offices, bouncers, kitchens, factories) are banned** — see non-negotiable 12b.
