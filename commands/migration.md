---
description: Change a schema, dependency, or storage layer
argument-hint: <intent...>
---

The user invoked `/aipe:migration` with intent: `$ARGUMENTS`.
...
3. Print: `✓ Scaffolded .aipe/. Edit .aipe/project/context.md, then re-run /aipe:migration $ARGUMENTS.`
...
## Step 3 — Load the `migration` template

Read the template at:

```
${CLAUDE_PLUGIN_ROOT}/specs/migration.md
```

If `${CLAUDE_PLUGIN_ROOT}` is unset (running from a dev clone), fall back to searching for `specs/migration.md` upward from this file's location.
...
Path: `.aipe/specs/migrations/<slug>.md`

If that file already exists, append an ISO timestamp: `.aipe/specs/migrations/<slug>-<YYYY-MM-DDTHH-MM-SS>.md`. **Never overwrite.**
