...
`study-system-design-dsa.md` is also the **structural foundation** of the whole study family — the per-concept-file template, formatting rules, diagram requirements, and hard rules live in this spec. The other topic generators (`/aipe:study-ai-engineering`, `/aipe:study-prompt-engineering`, `/aipe:study-agent-architecture`, `/aipe:study-interview-defense`) all read this file for structure.

To run every study generator in one pass instead of invoking each command by hand, use `/aipe:study` — the orchestrator.

**Scope.** This command covers system design and DSA only. AI engineering and ML have their own command (`/aipe:study-ai-engineering`), as do prompt engineering, agent architecture, and interview defense.
...
4. **How it works is the load-bearing block; it follows a three-move structure with frontend bridging.** Length scales with complexity, not capped at a paragraph count. Three required moves: Move 1 (the mental model, first paragraph + PATTERN diagram of the literal shape — loop / traversal frontier / topology / kernel, 5–12 lines) anchored to a frontend primitive; Move 2 (step-by-step walkthrough — one operation per line, one moving part per bolded sub-heading, **every sub-section gets at least one diagram**: pattern / layers-and-hops / execution trace); Move 2.5 (current state vs future state) when applicable; Move 3 (the principle, final paragraph) — the takeaway that generalises beyond this codebase.
    ...
    - **Use PSEUDOCODE for logic, not real code** — plain-English control flow, concrete variable names, one operation per line, annotated. **How it works carries no real code, no file paths, and no line references** — those live exclusively in Block 5 (Implementation in codebase). Keep the separation clean: Block 3 teaches the general pattern; Block 5 is where it lives in this repo.
...
5. **Primary diagram (Block 4) — the recap visual.** ONE primary diagram per concept (ASCII box-drawing in a fenced code block) that sits AFTER How it works. A reader who only looks at it should grasp the structure. Labels every architectural layer it spans.
6. **Implementation in codebase (Block 5 — renamed from "In this codebase").** Code side-by-side with line-by-line explanation: show actual repo code beside an annotation of what each part does. Must include real code references — `**File:**` + `**Function / class:**` + `**Line range:**` (e.g., `L42–L67`). For multi-file patterns, list every file with the role each plays. Never drop a raw block without annotation.
7. **Elaborate (Block 6).** Where this pattern comes from / The deeper principle / Where this breaks down / What to explore next.
8. **Interview defense (Block 8).** Each Q&A: What an interviewer is really asking / Likely questions (each labelled `[mid]` `[senior]` `[arch]`) / The question candidates always dodge / One-line anchors. Each question carries its own diagram sized to the question level.
9. **Validate (Block 9).** 4 levels — Reconstruct the diagram → Explain it out loud → Apply it to a new scenario → Defend the decision you'd change — plus a "Quick check — code reference test". Each level builds on the last; do not skip levels. Level 3 must reference the specific file and line range the reader checks their answer against.
10. **See also (Block 10, moved to last).** Cross-references via links; required reading order is not.
...
- "How it works contains real code, file paths, or line references" — move all real code + paths + line numbers to Block 5 (Implementation in codebase); replace with pseudocode + diagrams in Block 3
...
- "Implementation in codebase missing code side-by-side" — Block 5 requires actual repo code beside line-by-line annotation; not just file paths
