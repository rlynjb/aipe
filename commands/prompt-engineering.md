---
description: Fix AI output quality
argument-hint: <intent...>
---

The user invoked `/aipe:prompt-engineering` with intent: `$ARGUMENTS`.
...
3. Print: `✓ Scaffolded .aipe/. Edit .aipe/project/context.md, then re-run /aipe:prompt-engineering $ARGUMENTS.`
...
## Step 3 — Load the `prompt-engineering` template

Read the template at:

```
${CLAUDE_PLUGIN_ROOT}/specs/prompt-engineering.md
```

If `${CLAUDE_PLUGIN_ROOT}` is unset (running from a dev clone), fall back to searching for `specs/prompt-engineering.md` upward from this file's location.
...
Path: `.aipe/specs/prompts/<slug>.md`

If that file already exists, append an ISO timestamp: `.aipe/specs/prompts/<slug>-<YYYY-MM-DDTHH-MM-SS>.md`. **Never overwrite.**
