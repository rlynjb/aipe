...
The user invoked `/aipe:testing` with intent: `$ARGUMENTS`.
...
3. Print: `✓ Scaffolded .aipe/. Edit .aipe/project/context.md, then re-run /aipe:testing $ARGUMENTS.`
...
## Step 3 — Load the `testing` template

Read the template at:

```
${CLAUDE_PLUGIN_ROOT}/specs/testing.md
```

If `${CLAUDE_PLUGIN_ROOT}` is unset (running from a dev clone), fall back to searching for `specs/testing.md` upward from this file's location.
...
Path: `.aipe/specs/testing/<slug>.md`

If that file already exists, append an ISO timestamp: `.aipe/specs/testing/<slug>-<YYYY-MM-DDTHH-MM-SS>.md`. **Never overwrite.**
