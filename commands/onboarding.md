---
description: Generate context docs for a new codebase
argument-hint: <intent...>
---

The user invoked `/aipe:onboarding` with intent: `$ARGUMENTS`.
...
3. Print: `✓ Scaffolded .aipe/. Edit .aipe/project/context.md, then re-run /aipe:onboarding $ARGUMENTS.`
...
## Step 3 — Load the `onboarding` template

Read the template at:

```
${CLAUDE_PLUGIN_ROOT}/specs/onboarding.md
```

If `${CLAUDE_PLUGIN_ROOT}` is unset (running from a dev clone), fall back to searching for `specs/onboarding.md` upward from this file's location.
...
Path: `.aipe/specs/onboarding/<slug>.md`

If that file already exists, append an ISO timestamp: `.aipe/specs/onboarding/<slug>-<YYYY-MM-DDTHH-MM-SS>.md`. **Never overwrite.**
