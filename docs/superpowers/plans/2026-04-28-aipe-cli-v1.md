...
**Architecture:** TypeScript + Node 20 (ESM). Commander for CLI routing. Anthropic SDK for LLM calls (dependency-injected so it can be mocked in tests). Hybrid config: global at `~/.config/aipe/`, per-project at `.aipe/`. Templates source-of-truth lives at `specs/` (a sibling of `cli/`) and is copied into `cli/dist/templates/` at build time so it ships with the npm package.

**Repo layout:** monorepo with `cli/` (this work), reserved siblings `mcp/` (v2) and `web/` (v3), `html/` (existing static site), `specs/` (shared template source), `docs/`. The npm package published from `cli/` is named `aipe`.
...
- **Templates source:** `specs/` stays as the single source of truth, sibling to `cli/`. The CLI's `build` script copies it into `cli/dist/templates/`. The path resolver prefers `dist/templates/` (prod) and falls back to `../specs/` (dev).
...
> **⚠️ LAYOUT PIVOT (after the plan was first written):** the v1 CLI no longer lives at the repo root. It lives in `cli/`, with `mcp/` and `web/` reserved as siblings for future versions. `specs/` stays at the repo root as the shared template source, sibling to `cli/`. Read the actual target layout below — and prefix every later file path in this plan with `cli/`. All `npm`/`npx`/`node` commands run from `cli/`.
...
Templates live at `../specs/` (sibling of `cli/`). They cannot be referenced like that in a published npm package, so the `build` script copies `../specs/` into `cli/dist/templates/`. The path resolver tries both:

1. **Prod** (`node dist/cli.js`, or installed via npm): `dist/templates/` exists → use it.
2. **Dev** (`tsx src/cli.ts`): `dist/templates/` does not exist → fall back to `../specs/`.
...
**Files:**
- Modify: `specs/*.md` (13 files contain `.buffr` references)

- [ ] **Step 1: Confirm scope before editing**

```bash
grep -rn "\.buffr" specs/ | wc -l
```
...
```bash
find specs -name '*.md' -type f -exec \
...
Expected: 2 passed. (In dev — before `npm run build` — this passes via the fallback to `../specs/`.)
...
Expected: `dist/cli.js`, `dist/commands/*.js`, `dist/lib/*.js`, AND `dist/templates/*.md` (14 files copied from `../specs/`) all exist.
