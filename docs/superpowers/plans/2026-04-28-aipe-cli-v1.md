...
**Architecture:** TypeScript + Node 20 (ESM). Commander for CLI routing. Anthropic SDK for LLM calls (dependency-injected so it can be mocked in tests). Hybrid config: global at `~/.config/aipe/`, per-project at `.aipe/`. Templates source-of-truth lives at `prompting/specs/` (a sibling of `cli/`) and is copied into `cli/dist/templates/` at build time so it ships with the npm package.

**Repo layout:** monorepo with `cli/` (this work), reserved siblings `mcp/` (v2) and `web/` (v3), `html/` (existing static site), `prompting/specs/` (shared template source), `docs/`. The npm package published from `cli/` is named `aipe`.
...
- **Repo layout:** monorepo. Root-level HTML moves to `html/`. The CLI lives in `cli/` so future versions (`mcp/`, `web/`) can sit beside it.
- **Templates source:** `prompting/specs/` stays as the single source of truth, sibling to `cli/`. The CLI's `build` script copies it into `cli/dist/templates/`. The path resolver prefers `dist/templates/` (prod) and falls back to `../prompting/specs/` (dev).
...
> **⚠️ LAYOUT PIVOT (after the plan was first written):** the v1 CLI no longer lives at the repo root. It lives in `cli/`, with `mcp/` and `web/` reserved as siblings for future versions. `prompting/specs/` stays at the repo root as the shared template source, sibling to `cli/`. Read the actual target layout below — and prefix every later file path in this plan with `cli/`. All `npm`/`npx`/`node` commands run from `cli/`.

**Actual target layout:**

```
/Users/rein/Public/aipe/                ← repo root (monorepo)
  cli/                                  ← v1 (this plan's scope)
    bin/cli.js
    src/{cli,types}.ts, src/{commands,lib}/...
    tests/...
    dist/                               ← built (gitignored); contains templates/
    package.json, tsconfig.json, vitest.config.ts, .npmignore, README.md
  prompting/specs/                      ← shared template source
  html/                                 ← cheatsheet site (Task 0.1)
  docs/
  mcp/, web/                            ← reserved for v2/v3 (NOT this plan)
```

**Templates path resolution (used by `cli/src/lib/paths.ts`):**

Templates live at `../prompting/specs/` (sibling of `cli/`). They cannot be referenced like that in a published npm package, so the `build` script copies `../prompting/specs/` into `cli/dist/templates/`. The path resolver tries both:

1. **Prod** (`node dist/cli.js`, or installed via npm): `dist/templates/` exists → use it.
2. **Dev** (`tsx src/cli.ts`): `dist/templates/` does not exist → fall back to `../prompting/specs/`.

This way the CLI works both inside the monorepo (without running `npm run build` first) and as a published package.

---

## Conventions for every task

- **Working directory: every task in Phase 1 onward runs with cwd = `/Users/rein/Public/aipe/cli/`.** That includes `npm`, `npx`, `node`, `tsc`, `vitest`, `tsx`, AND `git`. Git auto-locates the `.git` in the repo root, so all `git add cli/foo` calls in commit blocks below should be read as `git add foo` (the path is already relative to `cli/`, so the `cli/` prefix is omitted in commit blocks for brevity).
- **Path convention in this plan:** when a task says "create `src/foo.ts`" or "test at `tests/foo.test.ts`", that means **`cli/src/foo.ts`** and **`cli/tests/foo.test.ts`** on disk. Inside `cli/`, the relative paths in code blocks are correct as written.
- **Exception:** Phase 0 tasks (already complete) run from the repo root.
...
- `--no-verify` is NEVER allowed.
- All TypeScript is ESM; use `import.meta.url` + `fileURLToPath` (Node 20.11+) — no CJS `__dirname`.
...
> **Working directory:** every task in Phase 1 onward runs from `cli/` unless explicitly stated otherwise. All file paths in these tasks are relative to `cli/`. The `npm`, `npx`, and `node` commands all assume cwd = `/Users/rein/Public/aipe/cli/`.

## Task 1.1: Initialise package and TypeScript

**Files:**
- Create: `cli/` directory and inside it: `package.json`, `tsconfig.json`, `vitest.config.ts`, `.npmignore`
- Modify: repo-root `.gitignore` to add `cli/node_modules/`, `cli/dist/`, `.env`, `*.tsbuildinfo`

- [ ] **Step 1: Create the cli/ directory and enter it**

```bash
mkdir -p /Users/rein/Public/aipe/cli
cd /Users/rein/Public/aipe/cli
```

All later commands in this task run from there.
...
- [ ] **Step 3: Replace generated package.json with the real shape**
...
The `scripts/copy-templates.js` referenced by `build` is created in step 8 below. `dist/` ships with templates already inside, so `files` does not need to list `prompting/`.
...
- [ ] **Step 7: Extend repo-root .gitignore**

This task does NOT create `cli/.gitignore`. It edits the existing repo-root `.gitignore` (which already has `.claude/worktrees/` and `.worktrees/`).

Append the following lines to `/Users/rein/Public/aipe/.gitignore`:
...
- [ ] **Step 8: Create the templates copy script**

Write `cli/scripts/copy-templates.js`:
...
- [ ] **Step 9: Create cli/.npmignore (belt-and-suspenders for files allowlist)**
...
- [ ] **Step 10: Verify install succeeded and the empty TypeScript project compiles**
...
From `cli/` (git auto-finds the repo's `.git`):

```bash
git add package.json package-lock.json tsconfig.json vitest.config.ts .npmignore scripts/copy-templates.js ../.gitignore
git commit -m "chore(cli): scaffold TypeScript package and tooling under cli/"
```

(`cli/node_modules/` and `cli/dist/` should already be ignored by the `.gitignore` edit in step 7.)
...
Expected: 2 passed. (In dev — before `npm run build` — this passes via the fallback to `../prompting/specs/`.)

- [ ] **Step 5: Commit**

```bash
git add src/lib/paths.ts tests/paths.test.ts
git commit -m "feat(paths): resolve package root and templates dir (dev + prod)"
```
...
```bash
mkdir -p /tmp/aipe-smoke && (cd /tmp/aipe-smoke && npx tsx /Users/rein/Public/aipe/cli/src/cli.ts init && ls -R .aipe) && rm -rf /tmp/aipe-smoke
```
...
```bash
mkdir -p /tmp/aipe-smoke-real && cd /tmp/aipe-smoke-real && npx tsx /Users/rein/Public/aipe/cli/src/cli.ts init
```
...
```bash
cd /tmp/aipe-smoke-real && npx tsx /Users/rein/Public/aipe/cli/src/cli.ts generate feature "add dark mode toggle" --dry-run
```
...
```bash
cd /tmp/aipe-smoke-real && ANTHROPIC_API_KEY=$ANTHROPIC_API_KEY npx tsx /Users/rein/Public/aipe/cli/src/cli.ts generate feature "add dark mode toggle"
```
...
## Task 8.1: `cli/README.md` for the published package

**Files:**
- Create: `cli/README.md` (this is the README npm includes when the package is published).

(Note: the repo-root `README.md` is a separate concern — it should describe the monorepo. This task only writes the package README that ships to npm. Keep the repo-root README as-is for now; it can be updated in a future task if needed.)

- [ ] **Step 1: Write `cli/README.md` with the content below.**
...
```bash
git add README.md
git commit -m "docs(cli): add package README for npm-published aipe CLI"
```
...
**Files:** none (verification only — produces `cli/dist/`)

All commands run from `/Users/rein/Public/aipe/cli/`.
...
Expected: `dist/cli.js`, `dist/commands/*.js`, `dist/lib/*.js`, AND `dist/templates/*.md` (14 files copied from `../prompting/specs/`) all exist.
...
```bash
mkdir -p /tmp/aipe-dist-smoke && (cd /tmp/aipe-dist-smoke && node /Users/rein/Public/aipe/cli/bin/cli.js init && ls .aipe/project) && rm -rf /tmp/aipe-dist-smoke
```
...
Expected: file list includes `bin/cli.js`, `dist/cli.js`, `dist/commands/...`, `dist/lib/...`, `dist/templates/*.md`, `package.json`, `README.md`. Should NOT include `src/`, `tests/`, `scripts/`, `tsconfig.json`, `vitest.config.ts`.
...
# if anything changed:
git add -A
git commit -m "chore(cli): verify build and packaging"
```
...
Expected: `0.1.0`. (CommonJS `require` won't work because the package is ESM; the dynamic import above is ESM-safe.)

- [ ] **Step 2: Tag (run from repo root)**

```bash
cd /Users/rein/Public/aipe
git tag -a aipe-v0.1.0 -m "aipe v0.1.0 — CLI release-candidate"
```

The tag is prefixed with `aipe-` so future v2 (`mcp/`) and v3 (`web/`) sub-packages can have independent version tags in the same monorepo.
