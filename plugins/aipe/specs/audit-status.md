...
> This spec is purely descriptive. If you want a debt list with triage, use `audit-cleanup.md`. If you want a frontend a11y read, use `audit-frontend-a11y.md`. If you want a staff-engineer refactor notebook, use `audit-refactor.md`. If you want to restructure code, use `refactor.md`. The audit-status job is to tell you *what is*, not *what to do*.
>
> The orchestrator `/aipe:audit` runs this spec alongside the other three audit-family specs in one pass. Run `/aipe:audit-status` directly when you only want the descriptive snapshot.
...
This file was originally `audit.md`. Renamed to `audit-status.md` when the `/aipe:audit` orchestrator was added — `audit.md` is now the orchestrator that runs `audit-status`, `audit-cleanup`, `audit-frontend-a11y`, and `audit-refactor` together. The output filename (`snapshot-<date>.md`) is unchanged and still describes the artifact better than the spec name would.
