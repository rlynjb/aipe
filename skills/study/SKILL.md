...
17. **Tech-stack rule — every concept file names both what the codebase uses AND the current industry leader.** Wherever a concept file references a tech choice (library, framework, service) — in How it works, Tradeoffs, Interview defense, or anywhere else — name two things: (a) **what this codebase actually uses** (the real library/framework/service in the repo; reference the actual file or import line); and (b) **what the leading industry choice is today** (2026, per the spec header), labelled as either **adoption-leading** (most-deployed-in-production across real teams today — what a senior engineer at a Series B startup would default to; use for battle-tested patterns like auth, request flow, DB access) or **innovation-leading** (the choice with the most mindshare and momentum, likely to lead in 1–2 years — what a senior engineer at a frontier-tech company is reaching for; use for fast-moving areas like AI tooling, edge compute, type safety). Each gets a one-sentence "why it leads" reason — not marketing, the specific technical thing that puts it in front (typed end-to-end, zero config, server-component-native, edge-runtime-compatible). Never claim a single tech "won"; call out real disagreement when it exists. **If the codebase already uses the leader**, name a credible runner-up to contrast against so the alternative landscape is still visible. This rule lets the reader build the same mental map you'd build by reading the discipline catalog at the end of the template — codebase choice paired with industry direction, every time.
...
Identify the patterns/operations to cover per section by walking the project context. **Use the template's "What to expect by discipline" catalog (loaded in Step 3) as the first pass**: determine whether the codebase is primarily frontend, backend, or full-stack, then walk the discipline's pattern list and note which ones you can ground in concrete code or context. The patterns you find become concept files; the ones you don't find are gaps the inventory can either skip (the codebase legitimately doesn't need them) or flag for the user to confirm. The cross-reference table ("if you see X, you're looking at Y") is also useful as a signal-to-pattern map when scanning the project context for evidence.
...
- **Sections structurally absent** — required template elements not present in the file at all. The current required structure, in order:
  ...
  - "Tech reference missing industry-leader pairing" — flag any concept file that names a specific library, framework, or service (e.g., Express, Prisma, Netlify Functions, React Query) without also naming the current industry leader for that category, labelled as **adoption-leading** or **innovation-leading**, with a one-sentence "why it leads" reason. If the codebase already uses the leader, the file must name a credible runner-up to contrast against. This rule applies file-wide; check every section (How it works, Tradeoffs, Interview defense) that names a tech.
  ...
  - **Note on legacy guides:**
    ...
    - Files generated under **v1.21.0** name specific libraries/frameworks/services (e.g., Express, Prisma, Netlify Functions, React Query) without pairing them with the current industry leader. v1.22.0 adds the tech-stack rule: every tech reference must be paired with what's adoption-leading or innovation-leading today (with a one-sentence "why it leads" reason), and if the codebase already uses the leader, a credible runner-up must be named. The v1.22.0 fix is to walk every section that names a tech (How it works, Tradeoffs sub-blocks, Interview defense answers and diagrams) and insert the industry-leader pairing alongside each name, drawing the leader/runner-up choices from the template's discipline catalog (loaded in Step 3 — frontend / backend / full-stack sections).
...
- For **tech references missing the industry-leader pairing**, walk every section that names a specific library / framework / service (How it works, Tradeoffs sub-blocks, Interview defense answers and diagrams) and add an industry-leader pairing alongside each name. The pairing has three parts: (1) what this codebase actually uses (the existing reference — leave it), (2) what's leading in the industry today, labelled either **adoption-leading** (most-deployed in production across real teams today — use for battle-tested patterns: auth, request flow, DB access) or **innovation-leading** (most mindshare/momentum, likely to lead in 1–2 years — use for AI tooling, edge compute, type safety), and (3) a one-sentence "why it leads" reason that names the specific technical thing (typed end-to-end, server-component-native, edge-runtime-compatible — not marketing). If the codebase already uses the leader, name a credible runner-up so the alternative landscape is still visible. Draw the leader/runner-up choices from the template's "What to expect by discipline" catalog (loaded in Step 3 — frontend / backend / full-stack sections each list "Common in real codebases" and "Leading today"). Never claim a single tech "won"; name real disagreement where it exists.
