...
12b. **Use real software, not analogies — applies to every block, every anchor.** Whenever the writer needs a concrete anchor for an abstract concept, reach for something the reader has actually used or built — not a physical-world metaphor. The reader is a working frontend engineer with 5–8 years of experience; their pattern recognition is far richer from software they touch daily than from coat checks or libraries.

    **Banned anchor types** (when used as the primary anchor): coat checks, librarians, locked doors, bouncers, matchmakers, post offices, notebooks, kitchens, factories, queues at coffee shops, traffic lights, mailboxes, key-on-a-keychain, etc.

    **Preferred anchor types**, in order of strength:
    - **Apps the reader uses daily** — Gmail's optimistic send, Linear's drag-to-reorder, Notion's per-user workspace isolation, ChatGPT's persistent system prompt, GitHub's branch protection rules, Figma's multi-cursor sync, Slack's threaded replies, Stripe's payment intent flow.
    - **DevTools and engineering surfaces the reader touches** — the network tab showing reused TCP connections, the "missing key prop" warning in React, the `X-RateLimit-Remaining` header in GitHub's API, the session cookie in application storage, Service Worker caching strategies, Next.js's `revalidatePath`.
    - **Patterns the reader has built in their own apps** — "You already do this when you check `req.user.id` in the route handler AND filter by `user_id` in the database query — that's two gates." "You already do this when you set `key={item.id}` on a list item in React."
    - **Industry-standard products, protocols, primitives** — Stripe's idempotency keys, JWT signatures, OAuth's authorization-code flow, Postgres MVCC, Redis `SETNX`, HTTP keep-alive, ETag headers, RFC 7234 cache-control directives.

    **The test:** could the reader open the app, the browser tool, or their own codebase and verify what the writer claims? If yes, the example is grounded. If the only way to verify is "trust me, this is like a coat check" — the writer used an analogy and needs to find a real example instead.
...
13. **Every concept file includes a "Why care" block** immediately after `**See also:**` and BEFORE `## How it works`. **Length scales with complexity** — a simple concept gets a short scenario plus a one-line summary; a complex one gets a fuller scenario, a worked before/after, and a bolded "why this matters" pivot. The block has **five structural moves**, in order:
    ...
    - **Move 1 — The grounded scenario.** Open with a concrete picture the reader can hold in their head — a real interaction with software they use, or a pattern they've already built. **Not abstract framing. Not a clever hook sentence. Not a physical-world analogy** (see rule 12b). Examples that work:
      - "Open Linear and drag an issue to a new spot in the list. The issue moves. Its comments, label, assignee — all still attached, in the new position. Now do it in two tabs simultaneously…"
      - "Open Gmail, type a message, hit send. The compose window closes immediately. But your wifi is flaky — the request might still be in flight five seconds later. Gmail doesn't make you wait."
      - "Open GitHub, push a commit to a branch with protection rules turned on. Before the merge can happen, two independent gates need to pass: a review from another engineer, and all status checks green."

      The reader is *remembering a real interaction*, not imagining a kitchen or a library. **Hook-sentence openings are banned**; **physical-world analogies are banned as primary anchors**. **Move 1 must be project-agnostic** — a reader who has never seen this codebase must understand the scenario.
    ...
    - **Move 5 — The one-line summary.** End with **one sentence** that names the pattern in a single phrase — a *reduction*, not a definition, and **not a physical-world metaphor**. Compress the concept into its essential job, ideally by naming the pattern as the same thing as something the reader already knows from real software. Examples:
      - "The reconciler is what React's `key` prop does, but for backend rows with attached metadata."
      - "The lock is `SETNX` — exactly that, no more."
      - "The connection pool is HTTP keep-alive, applied to database sockets instead of TCP."

      This is the sentence the reader paraphrases three weeks later, anchored to something they already know. Optionally follow with one handoff line to How it works ("Here's how that actually plays out in this codebase." / "The full mechanics are below.").
...
18. **How it works is the load-bearing block; it follows a three-move structure with frontend bridging.** Length scales with complexity, not capped at a paragraph count. A simple concept (debounce) gets four short paragraphs; a complex backend/AI/infra concept (multi-layer auth, distributed locks, prompt routing under failure) gets fifteen to twenty paragraphs with sub-headings, interspersed visuals, and explicit current-state vs future-state walkthroughs. The reader profile is a working frontend engineer (5–8 years React/Vue/TypeScript) pivoting toward full-stack and AI engineering — comfortable with components, hooks, client state, forms, browser APIs; still building intuition for database internals, transactional semantics, server-side request handling, auth flows, queues, distributed systems, LLM-shaped failure modes. The writer's job is to bridge from the first list to the second.
    ...
    - **Move 1 — The mental model (first paragraph).** Open with **a concrete picture anchored to real software the reader has used or built**, NOT a definition and NOT a physical-world analogy (see rule 12b). Examples:
      - Defense in depth → "This is GitHub's branch protection model: two independent gates that both have to pass — review approval AND status checks. Either alone wouldn't catch every bad merge; together, they catch what each one misses."
      - Connection pooling → "This is browser HTTP keep-alive, but for database sockets. Same TCP connection reused across many requests instead of handshaking every time."
      - Optimistic UI → "This is what Linear does when you check off an issue: the checkmark appears in 16ms, before the server has confirmed anything, and rolls back only if the network fails."

      After the mental model lands, one sentence names the underlying strategy in plain English ("two independent gates, both required" / "one warm resource, lent and returned" / "show success immediately, reconcile when the server confirms"). **Banned: locked doors, coat checks, librarians, post offices, bouncers, kitchens, factories** — see rule 12b for what to reach for instead.
...
Open with a concrete picture the reader can hold in their head — **a real interaction with software they use, or a pattern they've already built**. Not abstract framing. Not a clever hook sentence. **Not a physical-world analogy** (see non-negotiable 12b).

Examples of opening scenarios that work:

- "Open Linear and drag an issue to a new spot in the list. The issue moves. Its comments, its label, its assignee — all still attached, in the new position. Now do it again in a second tab. Both tabs reach for the same row at the same millisecond. Something has to decide which drag wins, and how the other client catches up."
- "Open Gmail, type a message, hit send. The message disappears from the compose window immediately. But your laptop's wifi is flaky — the request might still be in flight five seconds later. Gmail doesn't make you wait. It assumes success, shows you the sent folder, and reconciles later if the network fails. You've used this pattern in every modern app."
- "Open GitHub, push a commit to a branch with protection rules turned on. Before the merge can happen, two independent gates need to pass: a review from another engineer, and all status checks green. If either fails, the merge is blocked. The rules aren't a single 'is this allowed' check — they're two orthogonal checks that both have to pass."

What these scenarios have in common: they reference software the reader has used (Linear, Gmail, GitHub) with specific verifiable behaviors (drag-reorder, optimistic send, branch protection). The reader is *not imagining a kitchen or a library*; they're **remembering a real interaction**.

**Hook-sentence openings are banned.** "Most of the speed in a modern web app comes from not doing work" is clever but doesn't put a picture in the reader's head. **Physical-world analogies are banned** (coat checks, libraries, locked doors, post offices — see rule 12b). **Move 1 must be project-agnostic** — a reader who has never seen this codebase must understand the scenario.
...
> "That drag-reorder problem is what a reconciler solves. Not the diffing of the array, not the re-rendering — just the matching of old-row to new-row so the metadata can ride along. It's what React's `key` prop is for, on the frontend. Backends have the same problem and the same pattern name."

> "What decides which client wins is a *lock*. The same primitive Redis exposes as `SETNX`, the same thing Postgres advisory locks give you — a single value that only one process can hold at a time."

> "Those gates are *connection pools*. The browser doesn't open a fresh TCP handshake for every request you make to the same origin — it reuses one. Servers do the same thing with database connections, for the same reason."

Sharpening technique: **name what the pattern is by naming what it is NOT**. "Not the diffing, not the re-rendering — just the matching." Use it when the pattern sits adjacent to similar patterns it gets confused with.
...
End with **one sentence** that names the pattern in a single phrase — a **reduction**, not a definition, and **not a physical-world metaphor** (no matchmakers, no bouncers, no coat checks). Compress the concept into its essential job, ideally by naming the pattern as the same thing as something the reader already knows from real software.

> "The reconciler is what React's `key` prop does, but for backend rows with attached metadata."
> "The lock is `SETNX` — exactly that, no more."
> "The connection pool is HTTP keep-alive, applied to database sockets instead of TCP."

This is the sentence the reader will paraphrase three weeks later, anchored to real software they already know. Optionally follow with one **handoff line** that points to How it works: "Here's how that actually plays out in this codebase." / "The full mechanics are below."
...
Open with **a concrete picture anchored to real software the reader has used or built**, NOT a definition and NOT a physical-world analogy (see non-negotiable 12b). Examples:
...
**Banned: physical-world analogies.** Locked doors, coat checks, librarians, post offices, bouncers, kitchens — all banned as primary anchors. The reader has built apps; reach for their app-building knowledge before reaching for kitchen metaphors. See non-negotiable 12b for the catalog of preferred anchors.
...
- **Sections structurally absent** — required template elements not present in the file at all. The current required structure, in order:
  ...
  4. `**See also:**` line
  5. `## Why care` (restructured in v1.30.0 — was two paragraphs since v1.18.0; sharpened in v1.31.0 — physical-world analogies banned, real-software anchors required. Five structural moves with length scaling by complexity: Move 1 grounded scenario anchored to real software the reader has used or built (Linear, Gmail, GitHub, the reader's own React code), project-agnostic; Move 2 name the question/pattern, framed as the answer, often by reference to a related software pattern ("it's what React's `key` prop does, but…"); Move 3 bolded transition `**Why you need to answer that question at all:**` then paragraph naming concrete stakes — codebase references allowed here; Move 4 optional concrete before/after with bulleted scenarios; Move 5 one-line **summary** (renamed from "metaphor" in v1.31.0) that names the pattern by reference to real software, NOT a physical-world metaphor ("the reconciler is what React's `key` prop does, but for backend rows"). Hook-sentence openings, definition-first openings, and physical-world analogies are all banned. Banned phrases: "It's important to understand X", "X is a fundamental concept", "X is fascinating because.")
  6. `## How it works` (moved BEFORE the primary diagram in v1.20.0; restructured in v1.24.0; sharpened in v1.31.0 — physical-world analogies banned in Move 1's mental model, real-software anchors required. Length scales with complexity, not capped at a paragraph count. Required structure: Move 1 — mental model anchored to real software the reader has used or built (GitHub branch protection, HTTP keep-alive, Linear's optimistic checkmark, the reader's own auth-wall pattern), NOT a definition and NOT a physical-world analogy; Move 2 — layered walkthrough where each independent moving part gets its own bolded sub-heading and covers four things (the technical thing named with its real term, the bridge from what a frontend engineer already knows, the practical consequence walked through concretely, the condition under which it works/breaks); Move 2.5 — Phase A / Phase B sub-section when the concept involves something built-but-not-active, in-migration, or gradually rolling out; Move 3 — the principle that generalises beyond this codebase (NOT a summary). Every move-2 sub-heading must include a frontend-to-backend bridge sentence. Every abstract claim must be followed by a concrete consequence. Ends with an explicit handoff sentence to the primary diagram.)
  ...
  - "Why care or How it works uses a physical-world analogy as the primary anchor" — flag any Move 1 (Why care) or Move 1 (How it works) anchored to coat checks, librarians, locked doors, post offices, bouncers, kitchens, factories, notebooks, traffic lights, or any other physical-world metaphor. The v1.31.0 rule (non-negotiable 12b) bans these as primary anchors. The fix is to rewrite the anchor using real software the reader uses or has built (apps like Linear/Gmail/GitHub, DevTools surfaces, patterns like checking `req.user.id` in a route handler, industry primitives like `SETNX` or JWT signatures). The test: could the reader open the app, browser tool, or codebase and verify what the writer claims?
...
- For a **Why care or How it works using a physical-world analogy as the primary anchor** (locked doors, coat checks, librarians, post offices, bouncers, kitchens, notebooks, factories), rewrite the anchor using real software. The substitution table:
  - "Two locked doors" → "GitHub's branch protection: review approval AND status checks, both required" (defense in depth)
  - "A coat check" → "HTTP keep-alive for database sockets, the same TCP-reuse pattern but at the DB layer" (connection pooling)
  - "A librarian with cards" → same as above, drop the analogy
  - "The matchmaker" → "React's `key` prop, applied to backend rows with attached metadata" (reconciler)
  - "The bouncer at the door" → "Redis `SETNX` — a single value only one process can hold" (lock)
  - "Marking the envelope as sent" → "Linear's optimistic checkmark — the UI updates in 16ms before the server has confirmed" (optimistic UI)
  - "A notebook page being rewritten" → "drag-reordering a Linear issue list" or "React's `<TodoRow key={todo.id}>` pattern" (list reconciliation)

  The test: could the reader open an app, a browser tool, or their own codebase and verify the anchor? If yes, keep it. If only the writer can confirm "trust me, this is like a coat check," replace it.
