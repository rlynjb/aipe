---
name: audit-status
description: Take stock of a project — produce a status snapshot (one of the four generators run by /aipe:audit)
...
The user invoked `/aipe:audit-status` with optional scope: `$ARGUMENTS`.

> This is the **status-snapshot** generator. The orchestrator `/aipe:audit` runs this alongside `audit-cleanup`, `audit-frontend-a11y`, and `audit-refactor` in one pass. Run this command directly when you only want the descriptive status snapshot.
...
3. Print: `✓ Scaffolded .aipe/. Edit .aipe/project/context.md, then re-run /aipe:audit-status.`
...
## Step 3 — Load the `audit-status` template

Read the template at:

```
${CODEX_PLUGIN_ROOT}/specs/audit-status.md
```

If `${CODEX_PLUGIN_ROOT}` is unset (running from a dev clone), fall back to searching for `specs/audit-status.md` upward from this file's location.
...
6. **If the audit surfaces something that wants to be acted on, hand off** — debt → `/aipe:audit-status-cleanup`, restructuring → `/aipe:refactor`, new capability → feature work. Do NOT execute the action here.
