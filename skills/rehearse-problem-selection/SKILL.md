---
name: rehearse-problem-selection
description: Problem-selection rehearsal — justify why a problem deserves investment before designing a solution
---
...
The user invoked `/aipe:rehearse-problem-selection`.

This command takes **no arguments**. There is one `problem-selection rehearsal` artifact per repo at `.aipe/rehearse-problem-selection/`. Re-running enters UPDATE MODE when the artifact already exists.

## Step 1 — Initialize if needed

If `.aipe/project/context.md` does not exist, create `.aipe/project/` and `.aipe/specs/`, write a short project-context placeholder covering stack, data model, file structure, and must-not-change constraints, print `✓ Scaffolded .aipe/. Edit .aipe/project/context.md, then re-run /aipe:rehearse-problem-selection.`, and stop.
...
Check `.aipe/rehearse-problem-selection/` for `00-overview.md` or any numbered markdown artifact. Existing content means UPDATE; otherwise CREATE.
...
In CREATE mode, generate the complete artifact declared by `specs/rehearse-problem-selection.md`. In UPDATE mode, reconcile surgically against current code: add newly relevant material, update changed evidence, preserve correct teaching, and remove stale claims.
